...
// Basic browser PDF compressor using pdf-lib (structure compression only)
function Tab4PdfCompressor({ text }) {
  const [file, setFile] = React.useState(null);
  const [fileSize, setFileSize] = React.useState(0);
  const [ratio, setRatio] = React.useState(60);
  const [estimatedSize, setEstimatedSize] = React.useState(0);
  ...
  React.useEffect(() => {
    if (file) {
      ...
      setEstimatedSize(Math.round(file.size * (ratio / 100)));
      setDownloadUrl(null);
      setError("");
    } else {
      setFileSize(0);
  ...
  // Basic compression using pdf-lib (structure compression only)
  async function compressPdf(file) {
    ...
    try {
      if (!window.PDFLib || !window.PDFLib.PDFDocument) {
        setError("pdf-lib not loaded. Please include pdf-lib via CDN.");
      ...
      // Remove metadata (optional, for smaller size)
      pdfDoc.setTitle("");
      pdfDoc.setAuthor("");
      pdfDoc.setSubject("");
      pdfDoc.setKeywords([]);
      ...
      // pdf-lib does not support image downsampling or quality reduction,
      // but we can use compress: true and useObjectStreams: false for some reduction.
      ...
      setError("Compression failed: " + (e?.message || e));
      setCompressing(false);
    }
  }
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold text-blue-800 mb-4 text-center">
        {text?.pdfCompressorTitle || "PDF Compressor"}
      </h2>
      <div className="bg-white p-4 rounded shadow flex flex-col md:flex-row gap-6 items-start">
        <div style={{ flex: 1, minWidth: 220 }}>
          <label className="block mb-2 font-semibold">{text?.uploadPdf || "Upload PDF file"}</label>
          <input
            type="file"
          ...
          {file && (
            <div className="mb-2">
              <span className="font-semibold">{text?.selectedFile || "Selected File"}: </span>
              <span>{file.name}</span>
            </div>
          )}
          <div>
            <span className="font-semibold">{text?.originalSize || "Original Size"}: </span>
            <span>{file ? formatMB(fileSize) : "--"} MB</span>
          </div>
        </div>
        <div style={{ flex: 1, minWidth: 220 }}>
          <label className="block mb-2 font-semibold">
            {text?.compressionRatio || "Compression Ratio"}
          </label>
          <input
            type="range"
            min={10}
          ...
          <div>
            <span className="font-semibold">{text?.estimatedSize || "Estimated Output"}: </span>
            <span>
              {file ? formatMB(estimatedSize) : "--"} MB
            </span>
          </div>
          <div className="mt-4 text-gray-500 text-xs">
            {text?.noteOnlineOnly || "Note: Compression is browser-based and does not upload your file."}
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 text-center">
        <button
          className={`bg-blue-500 text-white px-6 py-2 rounded font-bold ${(!file || compressing) ? "opacity-60 cursor-not-allowed" : "hover:bg-blue-600"}`}
          disabled={!file || compressing}
          onClick={handleCompress}
        >
          {compressing
            ? (text?.compressing || "Compressing...")
            : (text?.compressBtn || "Compress & Download")}
        </button>
        {downloadUrl && (
          <div className="mt-4">
            <a
              ...
              className="bg-green-500 text-white px-5 py-2 rounded font-bold hover:bg-green-600"
            >
              {text?.downloadBtn || "Download Compressed PDF"}
            </a>
          </div>
        )}
      </div>
        ...
        <>
          <div className="max-w-2xl mx-auto p-4">
            ...
            <div style={{ marginTop: "3rem", textAlign: "center" }}>
              ...
              {activeTab === "tab4" && <Tab4PdfCompressor text={text} />}
            </div>
          </div>
          {/* Feedback Button */}
          <button
